...
public class Engine {
	...
	public Set<MoveSequence> possibleMoves(MoveSequence moveSequence){
		...
		if(moveSequence.board().barX > 0){
			for(Integer roll : moveSequence.remainingRolls().stream().distinct().collect(Collectors.toList())){
				if(moveSequence.board().points[roll-1].occupiedBy != CheckerColors.O || moveSequence.board().points[roll-1].amtCheckers == 1){
					List<Move> tempMoves = new ArrayList<>();
					tempMoves.addAll(moveSequence.moves());
					tempMoves.add(new Move(CheckerColors.X, roll, -1, roll-1));
			...
			for(Integer roll : moveSequence.remainingRolls().stream().distinct().collect(Collectors.toList())){
				...
				for(int i = 18; i <=23; i++){
					...
					if(moveSequence.board().points[i].occupiedBy == CheckerColors.X){
						if(i+roll <= 23 && (moveSequence.board().points[i+roll].occupiedBy != CheckerColors.O || moveSequence.board().points[i+roll].amtCheckers == 1)){
							List<Move> tempMoves = new ArrayList<>();
							tempMoves.addAll(moveSequence.moves());
							tempMoves.add(new Move(CheckerColors.X, roll, i, i+roll));
							List<Integer> remainingRolls = new ArrayList<>();
							remainingRolls.addAll(moveSequence.remainingRolls());
							remainingRolls.remove(Integer.valueOf(roll));
						...
						if(i+roll == 24 || (rollTooBig && !moveAlreadyUsed)){
							List<Move> tempMoves = new ArrayList<>();
							tempMoves.addAll(moveSequence.moves());
							tempMoves.add(new Move(CheckerColors.X, roll, i, 24));
							List<Integer> remainingRolls = new ArrayList<>();
							remainingRolls.addAll(moveSequence.remainingRolls());
							remainingRolls.remove(Integer.valueOf(roll));
						...
						if(i-roll < 24 && moveSequence.board().points[i-roll].occupiedBy == CheckerColors.O && moveSequence.board().points[i-roll].amtCheckers > 1) moveAlreadyUsed = true;
					}
				}
			}
		}
	...
	public Board doMoveForX(Board before, int from, int to){
		...
		if(from == -1 && after.barX > 0 && after.points[to].occupiedBy != CheckerColors.O){
			after.barX--;
			after.points[to].amtCheckers++;
			after.points[to].occupiedBy = CheckerColors.X;
			return after;
		}
		if(from == -1 && after.barX > 0 && after.points[to].occupiedBy == CheckerColors.O){
		...
		if(after.points[from].occupiedBy != CheckerColors.X || after.points[from].amtCheckers == 0) return null;
		if(to == 24){
			if(--(after.points[from].amtCheckers) == 0) after.points[from].occupiedBy = CheckerColors.NA;
			after.trayX++;
			return after;
		}
		if(after.points[to].occupiedBy == CheckerColors.O && after.points[to].amtCheckers > 1) return null;
		if(--(after.points[from].amtCheckers) == 0) after.points[from].occupiedBy = CheckerColors.NA;
		if(after.points[to].occupiedBy == CheckerColors.O){
			after.points[to].occupiedBy = CheckerColors.X;
			after.barO++;
			return after;
		}
